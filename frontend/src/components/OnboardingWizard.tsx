...
export function OnboardingWizard({ onComplete }: { onComplete: () => void }) {
  ...
  const [form, setForm] = useState({
    category: "psicologo", phone: "", registrationNumber: "", specialty: "",
    sessionDuration: 50, sessionPrice: 0, clinicName: "", clinicCity: "", clinicState: "", onlineService: true,
  });
  ...
  const handleFinish = async () => {
    try {
      ...
      toast.success("Bem-vindo ao Espaço Terapêutico!");
      onComplete();
    } catch {
      // Even if API fails, mark as done so user isn't stuck
      localStorage.setItem("onboarding_done", "true");
      onComplete();
    }
  };
    ...
    <div className="fixed inset-0 z-[60] bg-background flex items-center justify-center p-4">
      <div className="w-full max-w-md">
        <div className="flex items-center justify-center gap-2 mb-8">
          {[0, 1, 2].map(i => (
            <div key={i} className={`h-1.5 rounded-full transition-all ${i <= step ? "bg-brand-600 dark:bg-brand-300 w-12" : "bg-surface-300 dark:bg-surface-700 w-8"}`} />
          ))}
        </div>
        ...
        <div className="glass-card p-8">
              ...
              <div className="space-y-2">
                {CATEGORIES.map(c => (
                  <button key={c.value} onClick={() => { setForm(f => ({ ...f, category: c.value })); setStep(1); }}
                    className={`w-full flex items-center gap-3 p-3 rounded-xl border text-left transition-all ${form.category === c.value ? "border-brand-600 dark:border-brand-300 bg-brand-50 dark:bg-brand-600/10" : "border-surface-200 dark:border-surface-700 hover:border-brand-300"}`}>
          ...
          {step === 1 && (
            <div>
              <h2 className="text-xl font-semibold mb-2 text-center">Dados profissionais</h2>
              <p className="text-sm text-surface-500 mb-6 text-center">Informações básicas</p>
              <div className="space-y-3">
                ...
                <div><label className="text-xs font-medium text-surface-500 mb-1 block">CRP / Registro</label><input value={form.registrationNumber} onChange={e => setForm(f => ({ ...f, registrationNumber: e.target.value }))} className="input-premium w-full" /></div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div><label className="text-xs font-medium text-surface-500 mb-1 block">Duração (min)</label><input type="number" value={form.sessionDuration} onChange={e => setForm(f => ({ ...f, sessionDuration: +e.target.value }))} className="input-premium w-full" /></div>
                  <div><label className="text-xs font-medium text-surface-500 mb-1 block">Valor (R$)</label><input type="number" step="0.01" value={form.sessionPrice} onChange={e => setForm(f => ({ ...f, sessionPrice: +e.target.value }))} className="input-premium w-full" /></div>
                </div>
              </div>
            </div>
          )}
          ...
          {step === 2 && (
            <div>
              ...
              <p className="text-sm text-surface-500 mb-6 text-center">Opcional</p>
              <div className="space-y-3">
        ...
        <div className="flex justify-between mt-6">
          {step > 0 ? <button onClick={() => setStep(s => s - 1)} className="btn-ghost">Voltar</button> : <button onClick={skip} className="btn-ghost text-xs">Pular</button>}
          {step < 2 ? (
