...
import { transcriptions, globalConfig, patients } from '../db/schema.js';
import { eq, and, desc } from 'drizzle-orm';
import { authGuard } from '../middleware/auth.js';

function getConfig(key: string): string {
  const row = db.select().from(globalConfig).where(eq(globalConfig.key, key)).get();
  return row?.value || '';
}

export async function transcriptionRoutes(app: FastifyInstance) {
  // ── List transcriptions for a patient ─────────────────────
  app.get('/api/transcriptions', { preHandler: authGuard }, async (req) => {
    const { patientId, limit = '20' } = req.query as any;
    let conditions = [eq(transcriptions.ownerId, req.user.userId)];
    if (patientId) conditions.push(eq(transcriptions.patientId, patientId));
    return db.select().from(transcriptions).where(and(...conditions)).orderBy(desc(transcriptions.createdAt)).limit(parseInt(limit)).all();
  });

  // ── Save manual transcription ─────────────────────────────
  app.post('/api/transcriptions', { preHandler: authGuard }, async (req, reply) => {
    ...
    const [item] = db.insert(transcriptions).values({
  ...
  app.post('/api/transcriptions/downsub', { preHandler: authGuard }, async (req, reply) => {
    const parsed = z.object({
    ...
    (async () => {
      try {
        ...
        db.update(transcriptions).set({ rawTranscription: text, status: 'done' }).where(eq(transcriptions.id, item.id)).run();
      } catch (err: any) {
        app.log.error('[Downsub] Error:', err.message);
        db.update(transcriptions).set({ status: 'error', rawTranscription: `Erro: ${err.message}` }).where(eq(transcriptions.id, item.id)).run();
      }
    })();
  ...
  app.post('/api/transcriptions/:id/analyze', { preHandler: authGuard }, async (req, reply) => {
    const { id } = req.params as { id: string };
    const trans = db.select().from(transcriptions).where(and(eq(transcriptions.id, id), eq(transcriptions.ownerId, req.user.userId))).get();
    ...
    db.update(transcriptions).set({ status: 'analyzing' }).where(eq(transcriptions.id, id)).run();
...
Responda APENAS JSON válido.`;
      ...
      db.update(transcriptions).set({ aiAnalysis: JSON.stringify(analysis), status: 'done' }).where(eq(transcriptions.id, id)).run();

      return { ...analysis, transcriptionId: id };
    } catch (err: any) {
      db.update(transcriptions).set({ status: 'error' }).where(eq(transcriptions.id, id)).run();
      return reply.status(500).send({ error: `Erro na análise: ${err.message}` });
    }
  });

  // ── Get transcription with analysis ───────────────────────
  app.get('/api/transcriptions/:id', { preHandler: authGuard }, async (req, reply) => {
    const { id } = req.params as { id: string };
    const item = db.select().from(transcriptions).where(and(eq(transcriptions.id, id), eq(transcriptions.ownerId, req.user.userId))).get();
