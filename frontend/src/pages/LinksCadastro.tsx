...
import { Link2, Copy, Check, Trash2, Clock, UserCheck, AlertTriangle } from "lucide-react";
...
export default function LinksCadastro() {
  ...
  const createLink = async () => {
    try {
      ...
      if (r.ok) {
        ...
        const url = `${window.location.origin}/cadastro/${d.id}`;
        setLinks(prev => [{ ...d, url }, ...prev]);
  ...
  const copyLink = (id: string) => {
    const url = `${window.location.origin}/cadastro/${id}`;
    navigator.clipboard.writeText(url); setCopied(id); setTimeout(() => setCopied(""), 2000); toast.success("Link copiado!");
  };

  const handleDelete = async () => {
    if (deleteTarget) {
      await fetch(`${API}/api/registration-links/${deleteTarget}`, { method: 'DELETE', headers: auth() });
      setLinks(prev => prev.filter(l => l.id !== deleteTarget));
      toast.success("Link removido"); setDeleteTarget(null);
    }
  };
    ...
    <MainLayout>
      ...
      {links.length === 0 ? (
        ...
        <div className="space-y-4 page-enter">
          {links.map(l => {
            const url = `${window.location.origin}/cadastro/${l.id}`;
            return (
            <div key={l.id} className="glass-card p-4">
              ...
              <div className="flex items-center gap-2 p-3 rounded-xl bg-amber-50 dark:bg-amber-500/5 border border-amber-200 dark:border-amber-500/15 mb-3">
                ...
                <button onClick={() => copyLink(l.id)} className="w-9 h-9 rounded-lg flex items-center justify-center bg-brand-600 text-white shrink-0">
                  {copied === l.id ? <Check size={14} /> : <Copy size={14} />}
                </button>
              </div>

              <div className="flex items-center gap-6 text-xs text-surface-500 mb-3">
                <span>Criado: {fmt(l.createdAt)}</span>
              ...
              <div className="flex items-center gap-2 mt-2">
                <button onClick={() => copyLink(l.id)} className="btn-primary h-8 text-xs flex items-center gap-1.5"><Copy size={12} /> Copiar Link</button>
